	...
	public void pluginMessage(PluginMessageEvent event) throws IOException
	{
        ...
        if(task.equals(StaticValues.SAFE_CHECKPATH))
        {
        	...
        	if(!serverIsInhabitant(servers))
        	{
		        ...
		        player.get().getCurrentServer().get().getServer().sendPluginMessage(StaticValues.SAFE_TOSPIGOT, streamout.toByteArray());
				return;
        	}
        	...
        	case WARP:
		        ...
		        server.get().sendPluginMessage(StaticValues.SAFE_TOSPIGOT, streamout.toByteArray());
        	}
        } else if(task.equals(StaticValues.SAFE_CHECKEDPATH))
        {
	        ...
	        player.get().getCurrentServer().ifPresent(y -> y.sendPluginMessage(StaticValues.SAFE_TOSPIGOT, streamout.toByteArray()));
	        	;
        }
	}
